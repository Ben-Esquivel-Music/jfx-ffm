...
/*
 * Platform independent .h file generated by javah. Only GLPixelFormat_Attributes
 * still declares @Native constants; the GLContext and GLFactory classes were
 * migrated to the prism_es2 C ABI and no longer declare native methods, so their
 * JNI headers are no longer generated and are no longer included here.
 */
