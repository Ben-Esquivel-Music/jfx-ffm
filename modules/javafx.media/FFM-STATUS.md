...
* **macOS.** `mac.cmake`, the AVF backend (`jfxm_avf_*` in `OSXPlatform.mm` / `OSXMediaPlayer.mm`),
  the `CPlayerEventDispatcher*` ivar type change in `AVFMediaPlayer`, the band-holder release the
  `AVFAudioSpectrumUnit` destructor gained, and the Java `OSXPlatform` / `OSXMedia` /
  `OSXMediaPlayer` flips have never been compiled: this is a Windows machine with no Xcode. A
  macOS build or a CI run must confirm them.
...
| `platform/gstreamer/GstAudioSpectrum.cpp:99-101` | `SetBands` reads `m_pHolder` and then writes it instead of exchanging it | Two threads calling `setBandCount()` on one spectrum at once can both release the same superseded holder - a double free, and the release upcall on freed memory - while one of the two new holders is never released. `NativeAudioSpectrum` serialises today, so it is unreachable; `g_atomic_pointer_exchange` is the fix if that stops being true. Noted in contract section 11, because the ABI's exactly-once promise rests on the assumption |
